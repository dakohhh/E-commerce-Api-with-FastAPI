...
async def get_products_from_cart(user_id:str, db:Session):
    products_from_carts = db.query(product_table).join(cart_table).filter(cart_table.user_id == user_id).all()

    return products_from_carts
